...
# my_recommendations_redis = Redis(app, config_prefix="REDIS_MY_RECOMMENDATIONS")
...
# catalog.upload_recommendations(my_recommendations_redis.connection, "MY_RECOMMENDATIONS_FILE_PATH")

catalog2 = Catalog(app).load(app.config["TRACKS_CATALOG2"], app.config["TOP_TRACKS_CATALOG"],
                            app.config["TRACKS_WITH_DIVERSE_RECS_CATALOG"])
catalog2.upload_tracks(tracks_redis.connection, tracks_with_diverse_recs_redis.connection)
catalog2.upload_artists(artists_redis.connection)
catalog2.upload_recommendations(recommendations_redis.connection, "RECOMMENDATIONS_FILE_PATH")
catalog2.upload_recommendations(recommendations_svd_redis.connection, "RECOMMENDATIONS_SVD_FILE_PATH")
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            recommender = recommender = MyContextual(tracks_redis.connection, catalog2)
        else:
            recommender = recommender = Contextual(tracks_redis.connection, catalog)
